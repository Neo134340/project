...
// กำหนด type สำหรับ item ในตะกร้า เพื่อเก็บจำนวนและวันที่เช่า/คืน
interface CartItem {
    outfit: Outfit;
    quantity: number;
...
const getCartItemsFromLocalStorage = (): CartItem[] => {
    try {
        ...
        if (storedCartItemsWithDetails) {
            const parsedItems: { [id: string]: { outfit: Outfit; quantity: number; frequently?: number } } = JSON.parse(storedCartItemsWithDetails);
            return Object.values(parsedItems).map(item => ({
                ...
                returnDate: null,
                isAvailable: true,
                frequently: parsedItems[item.outfit.id]?.frequently || 0 // อ่านค่า frequently
            }));
        }
        return [];
    } catch (error) {
...
const saveCartItemsToLocalStorage = (items: CartItem[]) => {
    try {
        localStorage.setItem("cartItemsWithDetails", JSON.stringify(items.reduce((acc: { [key: string]: { outfit: Outfit; quantity: number; frequently?: number } }, item) => {
            acc[item.outfit.id] = {
                outfit: item.outfit,
                quantity: item.quantity,
                frequently: item.frequently || 0 // บันทึก frequently
            };
            return acc;
        }, {})));
    } catch (error) {
...
export default function CartPage() {
    ...
    const removeFromCart = (id: string) => {
        const updatedCart = cartItems.filter((item) => item.outfit.id !== id);
        setCartItems(updatedCart);
    };

    const increaseQuantity = (id: string) => {
        const updatedCart = cartItems.map((item) =>
            item.outfit.id === id ? { ...item, quantity: item.quantity + 1 } : item
        );
        setCartItems(updatedCart);
    };
    ...
    const decreaseQuantity = (id: string) => {
        const updatedCart = cartItems.map((item) =>
            item.outfit.id === id ? { ...item, quantity: Math.max(1, item.quantity - 1) } : item
        ...
        <div className="min-h-screen bg-gradient-to-br from-pink-100 via-purple-200 to-indigo-300 p-6">
            ...
            <div className="mb-4 p-4 bg-white rounded-md shadow-sm">
                ...
                <div className="flex space-x-4">
                    <div>
                        <label htmlFor="globalRentDate" className="block text-gray-700 text-sm font-bold mb-1">วันที่เช่า:</label>
                        <input
                            type="date"
                            id="globalRentDate"
                    ...
                    <div>
                        <label htmlFor="globalReturnDate" className="block text-gray-700 text-sm font-bold mb-1">วันที่คืน:</label>
                        <input
                            type="date"
                            id="globalReturnDate"
                            className="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline text-sm"
            ...
            <ul>
                {cartItems.map((cartItem) => (
                    ...
                    >
                        <img src={cartItem.outfit.image} alt={cartItem.outfit.name} className="w-24 h-24 object-cover rounded mr-4" />
                        ...
                        <div className="flex items-center space-x-2">
                            <button
                                onClick={() => decreaseQuantity(cartItem.outfit.id)}
                            ...
                            <button
                                onClick={() => increaseQuantity(cartItem.outfit.id)}
                        ...
                        <button
                            onClick={() => removeFromCart(cartItem.outfit.id)}
            ...
            <div className="mt-6 py-4 border-t bg-white rounded-md shadow-sm p-4 flex justify-between items-center">
                <div>
                    ...
                    {globalRentDate && globalReturnDate && (
                        <p className="text-gray-600 text-sm mt-1">วันที่เช่า: {globalRentDate}, วันที่คืน: {globalReturnDate}</p>
                    )}
                    {!globalRentDate || !globalReturnDate && (
